---
tags:
  - Character
  - Draft
title: Sun Wukong
description: Character profile for Sun Wukong
aliases:
  - 孫悟空
  - Son Goku
created: 2026-02-14 23:30
lastmod: Sunday 22nd February 2026 12:09:50
publish: true
draft: true
role: Antagonistic
status: Deceased
enableToc: true
arc: []
---
%% Who is this character at their core? What single sentence captures what they represent thematically? %%

> [!abstract]
> The strongest in history, from the [[Age of Enlightenment]].

## Basic Info
- **Nickname(s)**: The Strongest, Monkey King, The Great Ape
- **Age**: 35
%% - **First Appearance**: [[]]  Which arc or scene do they first show up?  %%
- **Affiliation**: Loner, clan was fully eradicated early in his life

## Appearance
%% - **Height**: 
- **Build**:  %%
- **Key Features**: Monkey like appearance
%% - **Voice/Mannerisms**:  How do they carry themselves? Speech patterns, posture, any tics?  %%

## Personality & Motivation
- **Flaw**: Negligence
- **Strengths**: Insane [[Imaginary Power System|IPS]] knowledge
- **Defining Trait**: His presence disrupts everything
## [[Imaginary Power System|IPS Knowledge]]
- **[[IPS Levels|IPS Level]]**: 5
- **IPS Specialty**: %% What concept did they conceptualize? (electricity, fire, intangibility, etc.) %%
- **IPS Background**: %% Innate practitioner, learned late, self-taught? Affects starting reserve. %%
- **Signature Technique**: %% Their most defining move — name and brief effect. %%
### [[IPS Technique|IPS Techniques]]
- [[Uchūzangeki]]
- [[Endorphins]]
- [[Divine Foresight]]
#### [[Technique Burn-In|Burned-In Techniques]]: %% Techniques so practiced they activate with no actions. %%
-  [[Flying Slash]]
- [[Regeneration]]

%% ## Background %%
%% Full prose or bullet points, whatever helps you write from their POV later. Include formative events, family, where they grew up, and why they are the way they are. %%
